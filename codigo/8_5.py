...
# Ejercicio 5: ECG (Electrocardiograma - Modelo Analitico Exacto)
...
# =============================================================================
# 9. MATEMATICA FOURIER — EJERCICIO 5: ECG (Modelo Analitico Exacto)
#    Periodo T = 1  =>  w0 = 2*pi
#    Forma de onda ECG modelada por rampas lineales entre los puntos:
#      (t1, 0) -> (tp, h) -> (t2, 0)  para cada onda
#    Matriz de parametros: (t1, tp, t2, h)
#      Onda P: (0.10, 0.14, 0.18,  0.15)
#      Onda Q: (0.30, 0.32, 0.34, -0.10)
#      Onda R: (0.34, 0.36, 0.38,  1.00)
#      Onda S: (0.38, 0.40, 0.42, -0.25)
#      Onda T: (0.55, 0.65, 0.75,  0.30)
#    a0 = 0.098  (calculado analiticamenente)
#    Clamp DAC: [-0.3, 1.1]  =>  valor_dac = int(((fx_clamp+0.3)/1.4)*255)
# =============================================================================

T_math = 1.0
w0     = 2.0 * math.pi / T_math    # w0 = 2*pi
a0     = 0.098

# Parametros de cada onda del ECG: (t1, tp, t2, h)
ondas_ecg = [
    (0.10, 0.14, 0.18,  0.15),   # Onda P
    (0.30, 0.32, 0.34, -0.10),   # Onda Q
    (0.34, 0.36, 0.38,  1.00),   # Onda R
    (0.38, 0.40, 0.42, -0.25),   # Onda S
    (0.55, 0.65, 0.75,  0.30),   # Onda T
]

def coef_an(n):
    """Coeficiente coseno calculado analiticament de la forma de onda ECG por rampas."""
    suma = 0.0
    wn   = n * w0
    wn2  = wn * wn
    for (t1, tp, t2, h) in ondas_ecg:
        m1  = h / (tp - t1)
        m2  = -h / (t2 - tp)
        suma += m1 * (math.cos(wn * tp) - math.cos(wn * t1))
        suma += m2 * (math.cos(wn * t2) - math.cos(wn * tp))
    return (2.0 / T_math) * suma / wn2

def coef_bn(n):
    """Coeficiente seno calculado analiticament de la forma de onda ECG por rampas."""
    suma = 0.0
    wn   = n * w0
    wn2  = wn * wn
    for (t1, tp, t2, h) in ondas_ecg:
        m1  = h / (tp - t1)
        m2  = -h / (t2 - tp)
        suma += m1 * (math.sin(wn * tp) - math.sin(wn * t1))
        suma += m2 * (math.sin(wn * t2) - math.sin(wn * tp))
    return (2.0 / T_math) * suma / wn2
...
def core0_generador_loop(nmax, lista_an, lista_bn):
    ...
    while True:
        ...
        # Clamp y mapeo DAC — ECG: rango aprox [-0.3, 1.1]
        fx_clamp  = max(-0.3, min(1.1, fx))
        valor_dac = int(((fx_clamp + 0.3) / 1.4) * 255)
        GPIO_SALP(valor_dac)
...
def ejecutar_generador():
    ...
    # nmax=30 para ECG: la forma de onda ya es fiel con 30 armonicos
    # y el calculo de coeficientes analiticos es rapido
    nmax = 30

    limpiar_area_trabajo(); dibujar_barra_estado()
    oled.text("GENERADOR", 28, 17, 1); oled.hline(0, 26, 128, 1)
    oled.text("Calc. ECG...", 12, 32, 1)
    oled.show()
    print("\nPrecalculando coeficientes ECG (nmax={})...".format(nmax))
    ...
    print("\n" + "=" * 42)
    print("  GENERADOR — EJ5 ECG")
    print("  T=1  w0=2pi  Modelo analitico P,Q,R,S,T")
    print("=" * 42)
...
def main():
    print("=" * 50)
    print("  LAB 8 — EJ5 ECG (ELECTROCARDIOGRAMA)")
    print("  GP14 btn_set  : avanza modo (1->2->3->4->5->1)")
    print("  GP15 btn_reset: vuelve a Modo 1")
    print("=" * 50)

    oled.fill(0)
    oled.text("LAB 8 - EJ5", 16, 4, 1)
    oled.text("ECG", 52, 16, 1)
    oled.hline(0, 28, 128, 1)
    oled.text("T=1  w0=2pi", 0, 35, 1)
