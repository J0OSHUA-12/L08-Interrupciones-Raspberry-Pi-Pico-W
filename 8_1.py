...
# Ejercicio 1: Función Mixta (PROP 3 del Lab 7)
...
# =============================================================================
# 9. MATEMATICA FOURIER — EJERCICIO 1: FUNCION MIXTA
#    Definida en [-2pi, 2pi]:
#      f(x) = x           para x en [-2pi, 0)
#      f(x) = 1 + x       para x en [0, pi/2)
#      f(x) = 1           para x en [pi/2, 2pi)
#    Periodo T = 4*pi  =>  w0 = 1/2
#    a0 = (1 + pi) / 2
#    an = [2*(-1)^n - n*sin(n*pi/2) - 4*cos(n*pi/2) + 2] / (pi*n^2)
#    bn = [n*((-1)^n - cos(n*pi/2) + pi*(1-(-1)^n)) - 4*sin(n*pi/2)] / (pi*n^2)
#    Rango aprox: [-0.27, 3.29]  => clamp [-0.5, 3.5]
# =============================================================================

T_math = 4.0 * math.pi
w0     = 2.0 * math.pi / T_math    # w0 = 0.5
a0     = (1.0 + math.pi) / 2.0

def coef_an(n):
    return (2*((-1)**n) - n*math.sin(n*math.pi/2) - 4*math.cos(n*math.pi/2) + 2) / (math.pi * n**2)

def coef_bn(n):
    return (n*(((-1)**n) - math.cos(n*math.pi/2) + math.pi*(1 - (-1)**n)) - 4*math.sin(n*math.pi/2)) / (math.pi * n**2)
...
# =============================================================================
# 11. FUNCIONES DE DISPLAY POR MODO (filas 15-63)
# =============================================================================
...
def core1_tareas():
    ...
    while True:
        ...
        if modo == 1:
            ...
            if not activo:
                gen_inicializado = False; utime.sleep_ms(50); continue
            ...
            lock.acquire()
            fx = datos_gen['fx']; t_actual = datos_gen['t']
            ...
            if gen_layout == 'fullscreen':
                ...
                for col in range(1, pw):
                    oled.line(px+col-1, buf_yp[col-1], px+col, buf_yp[col], 1)
                ...
                for row in range(1, GEN_PH):
                    oled.line(buf_xp[row-1], GEN_PY+row-1, buf_xp[row], GEN_PY+row, 1)
                dibujar_barra_estado(); oled.show()
        ...
        # ---- MODO 3: OHMETRO ----
            ...
            rx = leer_resistencia()
            lock.acquire()
            datos_multi['rx'] = rx; datos_multi['rx_str'] = formatear_resistencia(rx)
            lock.release()
        ...
        # ---- MODO 5: DHT22 ----
            ...
            if utime.ticks_diff(ahora, ultimo_dht) > 2000:
                ...
                if t_dht is not None:
                    datos_multi['temp_dht'] = t_dht; datos_multi['hum_dht'] = h_dht
...
# =============================================================================
# 13. CORE 0 — LOOP GENERADOR: calculo Fourier + DAC
# =============================================================================
...
def core0_generador_loop(nmax, lista_an, lista_bn):
    ...
    while True:
        ...
        # Clamp y mapeo DAC — Funcion Mixta: rango aprox [-0.27, 3.29]
        fx_clamp  = max(-0.5, min(3.5, fx))
        valor_dac = int(((fx_clamp + 0.5) / 4.0) * 255)
        GPIO_SALP(valor_dac)
...
def ejecutar_generador():
    ...
    print("\n" + "=" * 42)
    print("  GENERADOR — EJ1 FUNCION MIXTA")
    print("  T=4pi  w0=0.5  a0=(1+pi)/2")
    print("=" * 42)
...
def main():
    print("=" * 50)
    print("  LAB 8 — EJ1 FUNCION MIXTA")
    print("  GP14 btn_set  : avanza modo (1->2->3->4->5->1)")
    print("  GP15 btn_reset: vuelve a Modo 1")
    print("=" * 50)

    oled.fill(0)
    oled.text("LAB 8 - EJ1", 16, 4, 1)
    oled.text("FUNC. MIXTA", 16, 16, 1)
    oled.hline(0, 28, 128, 1)
    oled.text("T=4pi  w0=0.5", 0, 35, 1)
